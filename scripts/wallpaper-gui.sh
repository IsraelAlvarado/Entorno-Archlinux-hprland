...
# Combobox: el valor actual primero, el resto sin duplicarlo
TRANS_OPTS=(random fade wave wipe grow outer simple)
TRANS_REST=$(printf "%s\n" "${TRANS_OPTS[@]}" | grep -vxF "$TRANSITION" | paste -sd '!')
TRANS_CB="^${TRANSITION}!${TRANS_REST}"
...
FORM=$(yad --form \
    ...
    --width=650 --height=700 --center --borders=15 \
    --separator="|" --columns=1 \
    ...
    --field="Reiniciar Servicio:BTN" "pkill -f wallpaper-cycle.sh; pkill mpvpaper 2>/dev/null; $SCRIPT &" \
    --field="":LBL "" \
    --field="<span size='large' foreground='#a6e3a1'><b>󰋑 Transiciones y Tiempos</b></span>":LBL "" \
    --field="Efecto visual (Random = Evita repetir):CB" "$TRANS_CB" \
    --field="Segundos entre cambios:NUM" "${INTERVAL}!5..3600!5" \
    --field="Duración del efecto de transición (s):NUM" "${DURATION}!1..15!1" \
...
IFS="|" read -r \
    ...
    NEW_TRANSITION NEW_INTERVAL NEW_DURATION NEW_FPS NEW_SMART NEW_RANDOM \
    LBL4 LBL5 \
    NEW_COLLAGE NEW_COLLAGE_CHANCE NEW_COLLAGE_MIN NEW_COLLAGE_MAX \
    EXTRA <<< "$FORM"

NEW_TRANSITION=$(echo "$NEW_TRANSITION" | cut -d'^' -f1)

if [ "$NEW_COLLAGE_MIN" -gt "$NEW_COLLAGE_MAX" ]; then
    yad --error --title="Error" --width=300 --center \
        --text="El mínimo de imágenes del collage no puede superar al máximo." 2>/dev/null
    exit 1
fi

# =========================================================
# RECONVERSIÓN TRUE/FALSE → 1/0
# =========================================================
