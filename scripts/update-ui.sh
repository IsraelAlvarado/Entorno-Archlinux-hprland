...
run_with_progress() {
    ...
    (
        ...
        EXIT=$?
        echo "100"
        [ "$EXIT" -eq 0 ] && echo "# ✓ Completado" || echo "# ✗ Error detectado"
        sleep 0.7
...
show_log() {
    [ ! -f "$LOGFILE" ] && echo "Sin logs disponibles." > "$LOGFILE"
...
show_search() {
    ...
    /^[a-zA-Z0-9_.+-]+\// {
        ...
        if ((cmd | getline line) > 0) {
            split(line,b," ")
            if(b[2]==ver) state="✓ instalado"
            else state="󰚰 " b[2]
        }
        close(cmd)
        icon="󰣇"; tipo="🛡️ Oficial"
        if(repo=="aur") {
            icon="󰮯"
            if (pkg ~ /-bin$/) tipo="⚡ Binario (Rápido)"
            else if (pkg ~ /-(git|svn|hg|bzr|nightly)$/) tipo="🚧 Git/Dev (Inestable)"
            else tipo="📦 Fuente (Lento)"
        }
